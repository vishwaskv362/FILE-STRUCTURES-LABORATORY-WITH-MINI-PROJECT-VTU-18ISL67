...
#define filename "std2.txt" // create the empty file before execution
using namespace std;//not to be used in turbo C++ or in Ubuntu
fstream ifile;
...
int student::search()
{
	...
	while(!ifile.eof())
	 {
		flag=ifile.tellg(); // It returns the position of the current character in the output stream.
		unpack();
